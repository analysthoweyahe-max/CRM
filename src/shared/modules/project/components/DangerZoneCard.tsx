...
export function DangerZoneCard({ projectName, onDelete, isAr, deleteLabel, description }: Props) {
  ...
  const btnLabel  = deleteLabel  ?? (isAr ? 'حذف المشروع نهائياً' : 'Delete Project');
  const desc      = description  ?? (isAr
    ...
    <>
      <div className="rounded-2xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/10 p-6 space-y-3">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <Button variant="danger" startIcon={<Trash2 size={15} />} onClick={() => setOpen(true)}>
            {btnLabel}
          </Button>
