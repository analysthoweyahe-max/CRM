...
import { useWorkScopeContext } from '@/shared/modules/work-overview/hooks/useWorkScopeContext';
import { AttendanceExceptionRequestModal } from '../components/AttendanceExceptionRequestModal';
import {
  useEmployeeAttendanceExceptions,
  useCancelAttendanceException,
} from '../hooks/useEmployeeAttendanceExceptions';
import {
...
import type { UseWorkScopeOptions } from '@/shared/modules/work-overview/hooks/useWorkScopeContext';
...
export function MyAttendanceExceptionsPage({ layoutScope, seoRouteVariant }: UseWorkScopeOptions) {
  const { lang } = useLang();
  const isAr = lang === 'ar';
  const { routes } = useWorkScopeContext({ layoutScope, seoRouteVariant });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <Link to={routes.attendance} className="text-xs text-gray-400 hover:text-[#709028] mb-1 inline-block">
