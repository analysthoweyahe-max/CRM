...
import { usePersonalBonuses, useApiQueryError } from '../hooks/useWorkOverview';
...
export type PersonalBonusesPageProps = UseWorkScopeOptions;

export function PersonalBonusesPage({ layoutScope, seoRouteVariant }: PersonalBonusesPageProps) {
  const { lang } = useLang();
  const isAr = lang === 'ar';
  ...
  const { data, isLoading, error, isError } = usePersonalBonuses(scope, {
  ...
  return (
    <div className="space-y-5" dir={isAr ? 'rtl' : 'ltr'}>
      <PageHeader
        title={isAr ? 'مكافآتي' : 'My Bonuses'}
        subtitle={isAr ? 'عرض مكافآتك الشخصية فقط' : 'View your personal bonuses only'}
        actions={
      ...
      {isLoading && !data ? (
        ...
        <EmptyState
          title={isAr ? 'لا توجد مكافآت' : 'No bonuses'}
          description={isAr ? 'لا توجد مكافآت مسجّلة لهذا الشهر' : 'No bonuses recorded for this month'}
        />
      ) : rows.length > 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800/60 text-gray-500 dark:text-gray-400">
                  ...
                  <th className="px-4 py-2.5 text-start font-medium">{isAr ? 'ساعات إضافية' : 'OT hours'}</th>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/60">
                    <td className="px-4 py-2.5">
                      <Link to={routes.bonusDetail(r.id)} className="font-mono text-xs text-[#709028] dark:text-[#A0CD39] hover:underline">
                        {r.adjustmentDate || '—'}
                      </Link>
                    </td>
