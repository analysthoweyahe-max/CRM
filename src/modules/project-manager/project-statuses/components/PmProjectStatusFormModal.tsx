...
interface Props {
  open: boolean;
  onClose: () => void;
  onSubmit: (payload: CreatePmProjectStatusPayload) => void;
  initial?: ApiPmProjectStatus | null;
  isLoading: boolean;
  isAr: boolean;
}

const EMPTY: CreatePmProjectStatusPayload = {
  label_en: '', label_ar: '', color: '#6366f1',
  sort_order: 0, is_active: true, marks_completed: false,
};

export function PmProjectStatusFormModal({ open, onClose, onSubmit, initial, isLoading, isAr }: Props) {
  const [form, setForm] = useState<CreatePmProjectStatusPayload>(EMPTY);
  ...
  function set<K extends keyof CreatePmProjectStatusPayload>(field: K, value: CreatePmProjectStatusPayload[K]) {
    setForm((prev) => ({ ...prev, [field]: value }));
  }

  const isValid = !!(form.label_ar.trim() && form.label_en.trim());
