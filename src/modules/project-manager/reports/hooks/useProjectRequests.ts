...
import { pmRequestsApi } from '../api/requests.api';
import type { RawPmRequest } from '../api/requests.api';
import type { RequestItem, FilterKey } from '@/shared/modules/team-reports/types/teamReport.types';

const QUERY_KEY = ['pm-requests'];

function toRequestItem(r: RawPmRequest): RequestItem {
  return {
    id:            r.id,
...
export function useProjectRequests(isAr: boolean) {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: QUERY_KEY,
    queryFn:  () => pmRequestsApi.list({ per_page: 100 }).then(r => r.data.data.data.map(toRequestItem)),
  });

  const visible = filter === 'all' ? items : items.filter(r => r.status === filter);

  const approveMutation = useMutation({
    mutationFn: (id: string) => pmRequestsApi.approve(id),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: string) => pmRequestsApi.reject(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: QUERY_KEY });
