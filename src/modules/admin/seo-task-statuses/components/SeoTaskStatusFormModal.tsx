...
interface Props {
  open: boolean;
  onClose: () => void;
  onSubmit: (payload: CreateSeoTaskStatusPayload) => void;
  initial?: ApiSeoTaskStatus | null;
  isLoading: boolean;
  isAr: boolean;
}
...
export function SeoTaskStatusFormModal({ open, onClose, onSubmit, initial, isLoading, isAr }: Props) {
  const [form, setForm] = useState<CreateSeoTaskStatusPayload>(EMPTY);
  ...
  function set<K extends keyof CreateSeoTaskStatusPayload>(field: K, value: CreateSeoTaskStatusPayload[K]) {
  ...
  function handleSubmit() {
    if (!isValid) return;
    onSubmit({ ...form, label_ar: form.label_ar.trim(), label_en: form.label_en.trim() });
  }
