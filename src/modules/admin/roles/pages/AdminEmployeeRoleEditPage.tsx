...
export function AdminEmployeeRoleEditPage() {
  ...
  const { data: allPermissions } = usePermissionList('employee');
  ...
  function handleSave() {
    ...
    update({ id: role.id, payload: { name: role.name, permissions: filterRegisteredPermissions(permissions, registered), guard_name: 'employee' } }, {
      onSuccess: async () => {
        toast.success(isAr ? 'تم تحديث الدور' : 'Role updated');
        await refreshUser();
        navigate(ROUTES.ADMIN.EMPLOYEE_ROLES);
      },
      onError: (err) => {
        if (extractApiStatus(err) === 422) {
          toast.error(isAr
            ? 'الصلاحيات غير مهيأة على الخادم — يرجى التواصل مع المسؤول لتشغيل الـ seed'
            : 'Permissions not seeded on the server — contact admin to run the database seed');
          return;
        }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-3">
        <button
          type="button"
          onClick={() => navigate(ROUTES.ADMIN.EMPLOYEE_ROLES)}
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-800 dark:text-gray-100">
            {isAr ? 'تعديل دور الموظف' : 'Edit Employee Role'}
          </h1>
          <p className="text-sm mt-0.5 text-gray-500 dark:text-gray-400">
            {getRoleNameLabel(role.name, isAr)} · <span className="font-mono">{role.name}</span>
          </p>
        </div>
      </div>
