...
export function LeaveDetailPage() {
  ...
  const {
    ...
    approveNote, setApproveNote, rejectReason, setRejectReason,
    rejectError, setRejectError, isActionPending,
    handleApprove, handleReject,
  } = useLeaveDetailPage();

  const BackIcon = isAr ? ArrowRight : ArrowLeft;

  if (isLoading) return <div className="flex items-center justify-center py-24 text-gray-400 text-sm">{isAr ? 'جاري التحميل...' : 'Loading...'}</div>;
  if (isError || !req) return <div className="flex items-center justify-center py-24 text-gray-400 text-sm">{isAr ? 'الطلب غير موجود' : 'Request not found'}</div>;
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          <button onClick={() => navigate(ROUTES.LEAVES.LIST)}
        ...
        {req.status === 'pending' && (
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => setApproveOpen(true)} disabled={isActionPending}
              ...
              <CheckCircle size={15} />{isAr ? 'موافقة' : 'Approve'}
            </button>
            <button type="button" onClick={() => setRejectOpen(true)} disabled={isActionPending}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <Card padding="lg" className="flex flex-col gap-5">
          <div className="flex flex-col items-center text-center gap-2">
            ...
            <button type="button" onClick={() => navigate(ROUTES.EMPLOYEES.DETAIL(req.employee?.id ?? ''))}
          ...
          <div className="text-center">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">{isAr ? 'نوع الإجازة' : 'Leave Type'}</p>
            <span className="inline-block px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-300">
        ...
        <Card padding="lg" className="lg:col-span-2 space-y-5">
          ...
          {req.status === 'rejected' && req.rejection_reason && (
            <div>
              ...
              <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-xl px-4 py-3">{req.rejection_reason}</p>
            </div>
          )}
          {req.status === 'approved' && req.notes && (
            <div>
              <p className="text-xs text-[#709028] mb-1.5">{isAr ? 'ملاحظة' : 'Note'}</p>
              <p className="text-sm text-[#709028] bg-[#D8EBAE]/40 dark:bg-[#D8EBAE]/10 rounded-xl px-4 py-3">{req.notes}</p>
            </div>
          )}
          {req.attachment && (
            <div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-1.5">{isAr ? 'المرفقات' : 'Attachments'}</p>
              <div className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-3 bg-gray-50 dark:bg-gray-700/30">
      ...
      <ApproveModal open={approveOpen} onClose={() => setApproveOpen(false)} name={name}
        note={approveNote} onNoteChange={setApproveNote}
        onConfirm={handleApprove} isPending={isActionPending} isAr={isAr} />

      <RejectModal open={rejectOpen} onClose={() => { setRejectOpen(false); setRejectError(false); }} name={name}
        reason={rejectReason} onReasonChange={(v) => { setRejectReason(v); if (rejectError) setRejectError(false); }}
        hasError={rejectError} onConfirm={handleReject} isPending={isActionPending} isAr={isAr} />
    </div>
  );
}
